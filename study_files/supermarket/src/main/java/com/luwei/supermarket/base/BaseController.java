...
public class BaseController {
     ...
     * @Description: 获取对应地区等级
     * @Return: List<DistrictResponse>  返回类型
     * @Date: 2020/5/19 21:40
     */
    protected Notify<List<DistrictResponse>> getProvinceList(District.DistrictType districtType) {
        List<District> provinceList = districtService.findDistrictLevel(districtType);
    ...
    /**@Title: getCorrespondingType
     * @Description: 获取对应商品分类信息
     * @Param: [productCategoryType]   参数
     * @Return: ProductCategoryResponse  返回类型
     * @Date: 2020/5/23 0:49
     */
    protected Notify<List<ProductCategoryResponse>> getCorrespondingType(ProductCategory.ProductCategoryType productCategoryType){
        List<ProductCategory> productCategory = productCategoryService.findProductCategory(productCategoryType);
        ...
        if(!CollectionUtils.isEmpty(productCategory)){
            productCategory.forEach(p ->{
                ...
                productCategoryResponse.setValue(p.getId());
                productCategoryResponse.setLabel(p.getName());
                productCategoryResponse.setPrev(p.getPrev());
                productCategoryResponse.setType(p.getType().getIndex());
                productCategoryResponses.add(productCategoryResponse);
            });
        }
        return new Notify<>(Notify.Code.SUCCESS,productCategoryResponses);
    }
}
